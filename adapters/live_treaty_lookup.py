...
class StaticThenLiveTreatyTable:
    ...
    A live-lookup failure (network, API, unparseable response) falls back
    to the static table's own "not found" result rather than raising --
    a treaty-rate lookup failing should degrade to the existing statutory-
    default path, not break the determination.
    """
