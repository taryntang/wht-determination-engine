...
def test_upsert_determination_skip_reason_only():
    candidate = Candidate(
...
def test_upsert_determination_with_result():
    candidate = Candidate(
...
def test_set_review_decision_rejects_bad_status():
    with pytest.raises(ValueError):
        set_review_decision("det-1", "not_a_real_status", "Reviewer")


def test_set_review_decision_approved_builds_patch():
    with patch("urllib.request.urlopen", return_value=_mock_urlopen(b"[{}]")) as mock_open:
...
def test_set_review_decision_overridden_includes_rate_and_reasoning():
    with patch("urllib.request.urlopen", return_value=_mock_urlopen(b"[{}]")) as mock_open:
        set_review_decision(
            "det-1",
            "overridden",
